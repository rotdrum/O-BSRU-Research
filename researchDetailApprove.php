...
<?php 
  ...
  ?>
    ...
    ?>
            <p><?php echo $row2['student_card'] ; ?></p>
    <?php 
            $flag = 1 ;
        }
        }
    ...
    ?>
            ...
            <div class="btn-control-approve mt-50">
                <a href="teacher_researchApproveMyself.php?personal_id=<?php echo $_REQUEST['personal_id'] ; ?>" class="btn-approve btn-warning"><i class="fas fa-fast-backward"></i>ย้อนกลับ</a>
                    <?php 
                        if ($row['status1'] == 1) { 
                            $file = 1 ;
                    ?>
                        <a href="research/<?php echo $row['research_id'] ; ?>/<?php echo $row['file1'] ; ?>" class="btn-approve btn-info" target="blank" ><i class="fas fa-book"></i>ตรวจสอบบัณฑิตนิพนธ์</a>
                    <?php } 
                        else if( $row['status2'] == 1 ){ 
                            $file = 2 ;

                    ?>
                        <a href="research/<?php echo $row['research_id'] ; ?>/<?php echo $row['file2'] ; ?>" class="btn-approve btn-info" target="blank" ><i class="fas fa-book"></i>ตรวจสอบบัณฑิตนิพนธ์</a>
                    <?php } 
                        else if( $row['status3'] == 1 ){ 
                            $file = 3 ;

                    ?>
                        <a href="research/<?php echo $row['research_id'] ; ?>/<?php echo $row['file3'] ; ?>" class="btn-approve btn-info" target="blank" ><i class="fas fa-book"></i>ตรวจสอบบัณฑิตนิพนธ์</a>

                    <?php } 
                        else if( $row['status4'] == 1 ){ 
                            $file = 4 ;

                    ?>
                        <a href="research/<?php echo $row['research_id'] ; ?>/<?php echo $row['file4'] ; ?>" class="btn-approve btn-info" target="blank" ><i class="fas fa-book"></i>ตรวจสอบบัณฑิตนิพนธ์</a>

                    <?php 
                        }
                    ?>
                       <a  href="postresearch_notapprove.php?research_id=<?php echo $row['research_id']  ; ?>&file=<?php echo $file  ; ?>&personal_id=<?php echo $_REQUEST['personal_id'] ; ?>" class="btn btn-danger"><i class="fas fa-times-circle"></i>ไม่อนุมัติบัณฑิตนิพนธ์</a>
                    <a  href="postresearch_approve.php?research_id=<?php echo $row['research_id']  ; ?>&file=<?php echo $file  ; ?>&personal_id=<?php echo $_REQUEST['personal_id'] ; ?>" class="btn btn-success"><i class="fas fa-check-circle"></i>อนุมัติบัณฑิตนิพนธ์</a>
            </div>
