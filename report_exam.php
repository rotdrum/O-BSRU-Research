...
<section class="container">
      <div class="txt-header-manage">
        <h3 class="text-header">รายงานงานการจัดสอบ</h3>
        ...
        <a href="report_research.php?personal_id=<?php echo $_REQUEST['personal_id'] ; ?>" class="rp-select">บัณฑิตนิพนธ์</a>
          ...
          <a href="report_exam.php?personal_id=<?php echo $_REQUEST['personal_id'] ; ?>" class="rp-select rp-active">งานการจัดสอบ</a>
      ...
      <h1 class="title-print">รายงานงานการจัดสอบ</h1>
      <p id="shownone" style="display: none; width: 100%; text-align: center;">ไม่มีรายงานงานการจัดสอบ</p>
        ...
        <table class="table table-research" id="myTable">
            <thead>
              <tr>
                ...
                <th class="w-200">ชื่อ-สกุล</th>
                <th class="w-200">อาจารย์ที่ปรึกษา</th>
...
<script>
  $.ajax({
    type: "POST",
    dataType: "JSON",
    url: "./api/index/get_teacher_on_exam.php",
    data: {
      ...
      for(var i=0; i<response.result.length; i++, count++) {
        html += `
          <tr>
            ...
            <td>
              <p>${response.result[i].student_one}</p>`;
        response.result[i].student_two != "" || response.result[i].student_two != "null" ? html += `<p>${response.result[i].student_two}</p>` : "" ;
        html += `
            </td>
            <td>
              <p>${response.result[i].teacher1}</p>
              <p>${response.result[i].teacher2}</p>
              <p>${response.result[i].teacher3}</p>
            </td>
            <td>
